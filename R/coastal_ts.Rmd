...
Coeff.Summary.L3Eco <- rbind(
  Coeff.Summary.L3Eco,
  c(
  mean(log(CodaOne_L3Eco[,25])),
  sd(log(CodaOne_L3Eco[,25])),
  quantile(log(CodaOne_L3Eco[,25]),probs=c(0.025)),
  quantile(log(CodaOne_L3Eco[,25]),probs=c(0.975))
  )
)
rownames(Coeff.Summary.L3Eco)[26] <- "log_s" 
...
Alpha <- build_coefficient_matrix(Coeff.Summary.L3Eco[1:25,])
...
Pred.CatAll <- findPredictedClass.TSI(Evaluation,Coeff.Summary.L3Eco[1:25,])
...
plt <- generate_graphic_model(Model,Coeff.Summary.L3Eco[1:25,],"levelIII") +
...
pdf("figures/POLR_Prob_2010L3Eco(2022_03_01).pdf", width=6, height=6, paper="letter")
  print(plt)
dev.off()

data.frame(s=CodaOne_L3Eco[,25]) %>% 
  ggplot(aes(x=log(s)))+
  geom_histogram()+
  theme_classic()
...
pdf("figures/Posterior_Probability_Distributions_2010_(2022_03_01).pdf", width=6, height=6, paper="letter")
...
pdf("figures/Posterior_S_qnorm_(2022_03_01).pdf", width=6, height=6, paper="letter")
...
pdf("figures/Posterior_Probability_Trace_2010_(2022_03_01).pdf", width=6, height=6, paper="letter")
data.frame(CodaOne_L3Eco) %>% 
  mutate(x=row_number()) %>% 
