...
# did not ultimately use this for map
...
pdf(file="figures/Boston_Harbor_TS.pdf",useDingbats = FALSE,paper="letter",width=7,height=9)
  plt
dev.off()
png(file="figures/Boston_Harbor_TS.png",width=6,height=6,units="in",res=150)
  plt
dev.off()
...
bh <- cbind(bh,tmp)
save(bh,file="data/BostonHarborTSI.Rdata")
write.csv(bh,file="outputs/BH_TSI.csv",row.names=FALSE)
```
...
pdf(file="figures/QGAM figure.pdf",width=4,height=4,useDingbats = FALSE,paper="letter")
  chla_Qgam_plt  
dev.off()
```
...
This is an interesting plot illustrating how TSI related to Chlorophyll-a
in each 5 year period.  Chlorophyll-a levels associated with the highest TSI
values decreased a lot from 1995-2000, falling more inline with the 
relationship defined by the NCCA.

In other words, Chlorophyll-a during times of higher than normal nutrient levels was higher than expected, and fell to levels more consistent with 
the large scale relationship.
